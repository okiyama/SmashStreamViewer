 ...
 */
var globalJsonArray = new Array(); //Will contain the JSON data for making stream lists.
...
function requestStreamers() {
  ...
  for (var i = searchQueries.length - 1; i >= 0; i--) {
    ...
    req.open('GET', apiUrl + 'streams?limit=100&game=' + searchQueries[i], true);
...
function loadRunners(jsonArray) {
  ...
  for (var i = jsonArray.length - 1; i >= 0; i--) {
    dropdown = createDropdown(jsonArray[i]);
...
/**
 * Given json data, creates a dropdown for the data then returns it.
 *
 * @param
 */
function createDropdown(jsonData) {
  ...
  dropdown.setAttribute('isOpen', 'false');

  //No streams for this game, return an empty dropdown
  if (jsonData.streams.length == 0) {
    return dropdown;
  }

  var streamerList = loadStreamerList(jsonData);

  //It looks like this:
  // > Smash for Wii U (10 Streamers)
  //Then becomes:
  // V Smash for Wii U (10 Streamers)
  // Streams list
  dropdown.innerHTML = "> " + String(jsonData.streams[0].game);

  dropdown.appendChild(streamerList);
  dropdown.onclick = toggleDropdown(dropdown);
  return dropdown;
}

/**
 * Given a dropdown div, toggles if it is open or not.
 *
 */
function toggleDropdown(dropdown) 
{
  dropdown.setAttribute('isOpen', this.getAttribute('isOpen') === 'true' ? 'false' : 'true');
};
...
/**
 * Given a list of channels, returns a list sorted by number of viewers for each channel.
 *
 * @param{Array} channels The list of channels
 * @return The channels in sorted order, according to viewer count.
 */
function sortChannels(channels) {
  var sorted = channels.sort(
    function(a, b) {
...
/**
 * Whitelist of games to include
 * Check this before displaying a stream
 * @param game The name of the game
 *
 * @private
 */
function goodGame(game) {
  if (game == null) return false;
  if (game.search(/Super Smash Bros/i) > -1) {
    return true;
  }
  if (game.search(/Project M/i) > -1) {
    return true;
  }
  return false;
}
...
function setOpenLinkOnClicks() {
  twitchButton.onclick = function() {
    storeOpenLink(twitchButton);
  };
  twitchFSButton.onclick = function() {
    storeOpenLink(twitchFSButton);
  };
  srlButton.onclick = function() {
    storeOpenLink(srlButton);
  };
}
...
function setPageOnClicks() {
  ...
  settingsButton.onclick = function() {
    currentPageButton = swapPage(settingsButton, currentPageButton);
  };
  streamsButton.onclick = function() {
    currentPageButton = swapPage(streamsButton, currentPageButton);
  };
  aboutLink.onclick = function() {
    currentPageButton = swapPage(aboutLink, currentPageButton);
  };
}
